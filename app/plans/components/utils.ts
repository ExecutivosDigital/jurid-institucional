...
import type { PlanProps } from "types/global";
import type { Plan, BillingCycle } from "./types";

/** Converte o retorno de `GET /signature-plan` para o modelo usado pelo layout de planos. */
export function mapPlanPropsToPlan(p: PlanProps): Plan {
  const yd = (p.yearlyDiscount || 0) / 100;
  return {
    id: p.id,
    name: p.name,
    description: p.description ?? "",
    pixMonthlyPrice: p.pixPrice,
    pixYearlyPrice: p.pixPrice * 12 * (1 - yd),
    creditMonthlyPrice: p.creditCardPrice,
    creditYearlyPrice: p.creditCardPrice * 12 * (1 - yd),
    pixPrice: p.pixPrice,
